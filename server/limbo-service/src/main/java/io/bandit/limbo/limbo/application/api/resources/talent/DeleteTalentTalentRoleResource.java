...
import io.bandit.limbo.limbo.modules.main.talent.commands.TalentTalentRoleDelete;
...
@RestController
public class DeleteTalentTalentRoleResource {

    private static final String SWAGGER_DOC_TAG = "Talent";
    private static final String SWAGGER_DOC_DELETE_ONE_TALENT_ROLES = "Removes the talents for the current talent role.";
    ...
    @Inject
    public DeleteTalentTalentRoleResource(
    ...
    @ApiOperation(value = SWAGGER_DOC_DELETE_ONE_TALENT_ROLES, tags = {SWAGGER_DOC_TAG})
    @RequestMapping(value = TalentHttpRouter.TALENT_TALENTROLE_ROUTE, method = RequestMethod.DELETE, produces = MediaType.APPLICATION_JSON_VALUE)
    ...
    public Future<ResponseEntity<String>> delete(@PathVariable final String id) {
        ...
        return Mono.fromFuture(commandBus.execute(new TalentTalentRoleDelete.Command(id)))
    ...
    private ResponseEntity<String> deleteView(final Talent talent) {
        metrics.incrementTalentTalentRoleDeleted();
