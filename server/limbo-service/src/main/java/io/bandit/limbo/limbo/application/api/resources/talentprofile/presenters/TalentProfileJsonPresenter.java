...
@Named("TalentProfileJsonPresenter")
public class TalentProfileJsonPresenter implements TalentProfileOutputBoundary {

    private static final String NOT_FOUND_MESSAGE = "TalentProfile with id '%s' was not found.";
    ...
    private final TalentProfileHttpRouter router;

    public TalentProfileJsonPresenter(final ObjectMapper objectMapper, final TalentProfileHttpRouter router) {
        this.objectMapper = objectMapper;
        this.router = router;
    }

    public String success(final TalentProfile talentProfile) {
        try {
            final TalentProfileResponse response = TalentProfileResponse.fromTalentProfile(
                    talentProfile,
                    router.getTalentProfilesRoute(),
                    router.getTalentProfileRoute(talentProfile.getId()));
    ...
    public String notFound(final String talentProfileId) {
        try {

            final String message = String.format(NOT_FOUND_MESSAGE, talentProfileId);
    ...
    public String paginatedResponse(final String queryParams, final Paginated<TalentProfile> page) {
        try {
            final TalentProfilePaginatedResponse response = new TalentProfilePaginatedResponse(
            router.getTalentProfilesRoute(),
            queryParams,
            page);
