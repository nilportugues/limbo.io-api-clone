...
@Named("JobOfferJsonPresenter")
public class JobOfferJsonPresenter implements JobOfferOutputBoundary {

    private static final String NOT_FOUND_MESSAGE = "JobOffer with id '%s' was not found.";
    ...
    private final JobOfferHttpRouter router;

    public JobOfferJsonPresenter(final ObjectMapper objectMapper, final JobOfferHttpRouter router) {
        this.objectMapper = objectMapper;
        this.router = router;
    }

    public String success(final JobOffer jobOffer) {
        try {
            final JobOfferResponse response = JobOfferResponse.fromJobOffer(
                    jobOffer,
                    router.getJobOffersRoute(),
                    router.getJobOfferRoute(jobOffer.getId()));
    ...
    public String notFound(final String jobOfferId) {
        try {

            final String message = String.format(NOT_FOUND_MESSAGE, jobOfferId);
    ...
    public String paginatedResponse(final String queryParams, final Paginated<JobOffer> page) {
        try {
            final JobOfferPaginatedResponse response = new JobOfferPaginatedResponse(
            router.getJobOffersRoute(),
            queryParams,
            page);
