...
@Named("TalentTitleJsonPresenter")
public class TalentTitleJsonPresenter implements TalentTitleOutputBoundary {

    private static final String NOT_FOUND_MESSAGE = "TalentTitle with id '%s' was not found.";
    ...
    private final TalentTitleHttpRouter router;

    public TalentTitleJsonPresenter(final ObjectMapper objectMapper, final TalentTitleHttpRouter router) {
        this.objectMapper = objectMapper;
        this.router = router;
    }

    public String success(final TalentTitle talentTitle) {
        try {
            final TalentTitleResponse response = TalentTitleResponse.fromTalentTitle(
                    talentTitle,
                    router.getTalentTitlesRoute(),
                    router.getTalentTitleRoute(talentTitle.getId()));
    ...
    public String notFound(final String talentTitleId) {
        try {

            final String message = String.format(NOT_FOUND_MESSAGE, talentTitleId);
    ...
    public String paginatedResponse(final String queryParams, final Paginated<TalentTitle> page) {
        try {
            final TalentTitlePaginatedResponse response = new TalentTitlePaginatedResponse(
            router.getTalentTitlesRoute(),
            queryParams,
            page);
