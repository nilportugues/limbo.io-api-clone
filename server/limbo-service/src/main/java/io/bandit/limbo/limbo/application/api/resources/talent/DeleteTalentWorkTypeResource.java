...
import io.bandit.limbo.limbo.modules.main.talent.commands.TalentWorkTypeDelete;
...
@RestController
public class DeleteTalentWorkTypeResource {

    private static final String SWAGGER_DOC_TAG = "Talent";
    private static final String SWAGGER_DOC_DELETE_ONE_WORK_TYPES = "Removes the talents for the current work type.";
    ...
    @Inject
    public DeleteTalentWorkTypeResource(
    ...
    @ApiOperation(value = SWAGGER_DOC_DELETE_ONE_WORK_TYPES, tags = {SWAGGER_DOC_TAG})
    @RequestMapping(value = TalentHttpRouter.TALENT_WORKTYPE_ROUTE, method = RequestMethod.DELETE, produces = MediaType.APPLICATION_JSON_VALUE)
    ...
    public Future<ResponseEntity<String>> delete(@PathVariable final String id) {
        ...
        return Mono.fromFuture(commandBus.execute(new TalentWorkTypeDelete.Command(id)))
    ...
    private ResponseEntity<String> deleteView(final Talent talent) {
        metrics.incrementTalentWorkTypeDeleted();
