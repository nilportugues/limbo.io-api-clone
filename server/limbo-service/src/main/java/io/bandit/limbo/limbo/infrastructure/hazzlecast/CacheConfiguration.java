package io.bandit.limbo.limbo.infrastructure.hazzlecast;
...
import io.bandit.limbo.limbo.infrastructure.liquibase.DatabaseConfiguration;
...
public class CacheConfiguration {
    ...
    public HazelcastInstance hazelcastInstance() {
        ...
        config.setInstanceName("LimboApp");

        config.getNetworkConfig().setPort(5701);
        config.getNetworkConfig().setPortAutoIncrement(true);

        // In development, remove multicast auto-configuration
        if (env.acceptsProfiles(Constants.DEVELOPMENT)) {
    ...
    private MapConfig initializeDomainMapConfig() {
        ...
        mapConfig.setTimeToLiveSeconds(120); //TTL
        return mapConfig;
    }
