...
public class TalentCityDeleteMany {
    ...
    @Named("TalentCityDeleteMany.CommandHandler")
    public static class CommandHandler implements ICommandHandler<Command> {
        ...
        /**
         * Delete the current City value for a given set of Talents.
         */
        public CompletableFuture<List<Talent>> handle(final Command command) {
        //public Mono<List<Talent>> deleteCities(final Command command) {

            return CompletableFuture.supplyAsync(() -> {
                final List<Talent> list = new ArrayList<>();

                command.getTalentIds().forEach(talentId -> {
                    final Talent talent = talentRepository.findOneWithCity(talentId);
