...
public class TalentCountryDeleteMany {
    ...
    @Named("TalentCountryDeleteMany.CommandHandler")
    public static class CommandHandler implements ICommandHandler<Command> {
        ...
        /**
         * Delete the current Country value for a given set of Talents.
         */
        public CompletableFuture<List<Talent>> handle(final Command command) {
        //public Mono<List<Talent>> deleteCountries(final Command command) {

            return CompletableFuture.supplyAsync(() -> {
                final List<Talent> list = new ArrayList<>();

                command.getTalentIds().forEach(talentId -> {
                    final Talent talent = talentRepository.findOneWithCountry(talentId);
