...
import io.bandit.limbo.limbo.modules.main.talenttitle.queries.TalentTitlePaginated;
...
@RestController
public class GetPaginatedTalentTitleResource {

    private static final String SWAGGER_DOC_TAG = "Talent Title";
    private static final String SWAGGER_DOC_GET_MANY = "Get all the talent titles.";

    private final IQueryBus queryBus;
    private final TalentTitleQueryParams queryParams;
    private final TalentTitleJsonPresenter presenter;
    private final TalentTitleApiMetrics metrics;

    @Inject
    public GetPaginatedTalentTitleResource(final IQueryBus queryBus,
        final TalentTitleQueryParams queryParams,
    ...
    @RequestMapping(value = TalentTitleHttpRouter.TALENTTITLE_MANY_ROUTE, method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_VALUE)
    @ResponseStatus(value = HttpStatus.OK)
    @ApiResponses({
        @ApiResponse(code = 200, message = "OK", response = TalentTitlePaginatedResponse.class),
    ...
    public Future<ResponseEntity<String>> getAll() {
        ...
        return Mono.fromFuture(queryBus.dispatch(new TalentTitlePaginated.Query(fieldsOptions, filterOptions, pageOptions, sortOptions)))
            .map(page -> paginatedView((Paginated<TalentTitle>) page, request, pageOptions))
    ...
    private ResponseEntity<String> paginatedView(final Paginated<TalentTitle> page,
