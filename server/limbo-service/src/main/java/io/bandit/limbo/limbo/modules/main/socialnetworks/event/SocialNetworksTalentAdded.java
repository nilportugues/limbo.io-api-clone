...
public class SocialNetworksTalentAdded extends DomainEvent<SocialNetworks> {

    @JsonIgnore
    private static final Integer EVENT_VERSION = 1;

    @JsonIgnore
    public static final String EVENT_NAME = "social_networks.talent.added";

    public SocialNetworksTalentAdded(final SocialNetworks socialNetworks, final Talent talent) {
        setVersion(EVENT_VERSION);
        setType(EVENT_NAME);
        setPayload(buildPayload(socialNetworks, talent));
    }

    private Payload buildPayload(final SocialNetworks socialNetworks, final Talent talent) {

        if (!Optional.ofNullable(socialNetworks.getId()).isPresent()) {
            return null;
        }
        return new Payload(socialNetworks.getId(), talent.getId());
    }
