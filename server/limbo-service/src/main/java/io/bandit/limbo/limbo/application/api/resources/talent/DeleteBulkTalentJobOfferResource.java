...
import io.bandit.limbo.limbo.modules.main.talent.commands.TalentJobOfferDeleteMany;
...
@RestController
public class DeleteBulkTalentJobOfferResource {

    private static final String SWAGGER_DOC_TAG = "Talent";
    private static final String SWAGGER_DOC_DELETE_MANY_JOB_OFFERS = "Removes the talents for the current job offer.";
    ...
    @Inject
    public DeleteBulkTalentJobOfferResource(
    ...
    @ApiOperation(value = SWAGGER_DOC_DELETE_MANY_JOB_OFFERS, tags = {SWAGGER_DOC_TAG})
    @RequestMapping(value = TalentHttpRouter.TALENT_JOBOFFER_DELETE_BATCH_ROUTE, method = RequestMethod.DELETE, produces = MediaType.APPLICATION_JSON_VALUE)
    ...
    public Future<ResponseEntity<String>> deleteBulk(@PathVariable String id, @RequestBody String[] ids) {
        ...
        final TalentJobOfferDeleteMany.Command command = new TalentJobOfferDeleteMany.Command(id, Arrays.asList(ids));
    ...
    private ResponseEntity<String> deleteView(final Object object) {
        metrics.incrementTalentJobOfferDeleted();
