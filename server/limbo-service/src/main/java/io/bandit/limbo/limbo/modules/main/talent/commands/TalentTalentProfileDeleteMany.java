...
public class TalentTalentProfileDeleteMany {
    ...
    @Named("TalentTalentProfileDeleteMany.CommandHandler")
    public static class CommandHandler implements ICommandHandler<Command> {
        ...
        /**
         * Delete the current TalentProfile value for a given set of Talents.
         */
        public CompletableFuture<List<Talent>> handle(final Command command) {
        //public Mono<List<Talent>> deleteTalentProfiles(final Command command) {

            return CompletableFuture.supplyAsync(() -> {
                final List<Talent> list = new ArrayList<>();

                command.getTalentIds().forEach(talentId -> {
                    final Talent talent = talentRepository.findOneWithTalentProfile(talentId);
