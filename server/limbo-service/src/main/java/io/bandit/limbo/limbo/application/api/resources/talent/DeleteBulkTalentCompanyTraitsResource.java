...
import io.bandit.limbo.limbo.modules.main.talent.commands.TalentCompanyTraitsDeleteMany;
...
@RestController
public class DeleteBulkTalentCompanyTraitsResource {

    private static final String SWAGGER_DOC_TAG = "Talent";
    private static final String SWAGGER_DOC_DELETE_MANY_COMPANY_TRAITS = "Removes the talents for the current company traits.";
    ...
    @Inject
    public DeleteBulkTalentCompanyTraitsResource(
    ...
    @ApiOperation(value = SWAGGER_DOC_DELETE_MANY_COMPANY_TRAITS, tags = {SWAGGER_DOC_TAG})
    @RequestMapping(value = TalentHttpRouter.TALENT_COMPANYTRAITS_DELETE_BATCH_ROUTE, method = RequestMethod.DELETE, produces = MediaType.APPLICATION_JSON_VALUE)
    ...
    public Future<ResponseEntity<String>> deleteBulk(@PathVariable String id, @RequestBody String[] ids) {
        ...
        final TalentCompanyTraitsDeleteMany.Command command = new TalentCompanyTraitsDeleteMany.Command(id, Arrays.asList(ids));
    ...
    private ResponseEntity<String> deleteView(final Object object) {
        metrics.incrementTalentCompanyTraitsDeleted();
